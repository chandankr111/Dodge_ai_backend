...
// GUARDRAIL
// ================================================================
function isDatasetRelated(question) {
...
// ================================================================
function traceDocumentFlow(docNumber) {
    const db = getDb();
    const result = {
  ...
  `).all(docNumber);
    ...
    if (billingDoc || billingItems.length > 0) {
      ...
      `).get(billingItems[0].referenceSdDocument);
            if (deliveryDoc) {
                result.flow.push({
                    step: 3,
          ...
          WHERE deliveryDocument = ?
        `).all(deliveryDoc.deliveryDocument);
          ...
          `).get(deliveryItems[0].referenceSdDocument);
                    if (salesOrder) {
                        result.flow.push({
                            step: 4,
                            entity: 'Sales Order',
        ...
        LIMIT 10
      `).all(billingDoc.accountingDocument);
            paymentClearingDocs = Array.from(new Set(journalEntries
        ...
        if (billingDoc?.accountingDocument) {
            ...
            if (paymentClearingDocs.length > 0) {
                ...
                payments = db
                    .prepare(`SELECT * FROM payments_accounts_receivable
             WHERE clearingAccountingDocument IN (${placeholders})
             LIMIT 5`)
                    .all(...paymentClearingDocs);
  ...
  `).get(docNumber);
    ...
    `).all(docNumber);
        ...
        SELECT * FROM outbound_delivery_headers
        WHERE deliveryDocument = ?
      `).get(deliveryLinks[0].deliveryDocument);
            if (delivery) {
                result.flow.push({
                    step: 4,
                    entity: 'Outbound Delivery',
          ...
          WHERE referenceSdDocument = ?
        `).all(delivery.deliveryDocument);
                if (billingLinks.length > 0) {
                    const billing = db.prepare(`
            SELECT * FROM billing_document_headers
            WHERE billingDocument = ?
          `).get(billingLinks[0].billingDocument);
                    if (billing) {
                        result.flow.push({
                            step: 5,
                            entity: 'Billing Document',
              ...
              WHERE accountingDocument = ?
              LIMIT 10
            `).all(billing.accountingDocument);
                        result.flow.push({
                            step: 6,
                            entity: 'Journal Entry',
                            count: journalEntries.length,
                        ...
                        const paymentClearingDocs = Array.from(new Set(journalEntries
                            .map((j) => j.clearingAccountingDocument)
                            .filter((v) => typeof v === 'string' && v.trim() !== '')));
                        let payments = [];
                        if (paymentClearingDocs.length > 0) {
...
function generateFallbackSQL(question) {
    ...
    if (q.includes('highest number of billing documents') ||
...
// BROKEN FLOW QUERIES
// ================================================================
function getBrokenFlowSQL(question) {
    const q = question.toLowerCase();
    ...
    if (queryType === 'TRACE_FLOW') {
        ...
        if (docMatch && docMatch[1]) {
            ...
            if (flowData && flowData.flow.length > 0) {
                const answer = await summarizeResults(question, 'TRACE_FLOW', flowData);
                return {
                    answer,
                    sql: 'TRACE_FLOW',
                    data: flowData,
