...
function batchInsert(
    ...
    for (const row of rows) {
      ...
      try {
        ...
        if (errorCount <= 5) {
          ...
          console.warn(
            `  [seed:${label}] insert error (${errorCount}/${rows.length}):`,
            err?.message ?? err
          );
        }
      }
  ...
  // eslint-disable-next-line no-console
  console.log(
    `  ↳ ${label}: insert attempted=${insertAttempts}, errors=${errorCount}, changes=${insertedChanges}`
  );
}
...
function seed() {
  ...
  batchInsert(
    ...
    `),
    readJsonlFolder('product_descriptions'),
